...
The `wasm32-web-wabi` target is a WebAssembly compilation target for web
environments. It is based on [`wasm32-unknown-unknown`](./wasm32-unknown-unknown.md).
...
as `wasm32-unknown-unknown`. For example, filesystem, networking, process, and
thread APIs should not be assumed to work unless the standard library grows
explicit support for them.

## Building the target

To build this target, add `wasm32-web-wabi` to the `target` list in
`bootstrap.toml`.
...
```sh
$ WABI_SDK=/path/to/wabi-sdk cargo +nightly build -Zbuild-std --target wasm32-web-wabi
```
...
```sh
$ CARGO_TARGET_WASM32_WEB_WABI_RUNNER=wabi-runner \
    cargo +nightly test -Zbuild-std --target wasm32-web-wabi
```
...
Use `target_arch = "wasm32"` as an additional condition when code must
distinguish this target from `wasm64-web-wabi`.

## Enabled WebAssembly features

The default set of WebAssembly features enabled for compilation is currently the
same as [`wasm32-unknown-unknown`](./wasm32-unknown-unknown.md). See that
